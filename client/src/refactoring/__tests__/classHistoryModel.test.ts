...
describe('redo/restore result parsing', () => {
  ...
  it('reads a restore error', () => {
    const result = parseRevertResult(
      JSON.stringify({ reverted: false, error: 'index out of range' }),
    );
...
describe('remove-version result parsing', () => {
  ...
  it('reads a removal error (e.g. the current version)', () => {
    const result = parseRemoveResult(
      JSON.stringify({
        removed: false,
        error: 'cannot remove the current version',
      }),
    );
