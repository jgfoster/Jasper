...
class ListFilter extends HTMLElement {
  ...
  static #filterFor(listElement) {
    ...
    const filters = document.querySelectorAll(
      `${listFilterTag}[for="${CSS.escape(listElement.id)}"]`,
    );
    if (filters.length > 1) {
      throw new Error(
        `Found ${filters.length} list-filter elements for #${listElement.id} — only one filter per list is supported.`,
      );
    }
  ...
  connectedCallback() {
    this.injectStyles();
    this.innerHTML =
      '<input type="text" placeholder="Filter…" autocomplete="off" spellcheck="false"><button class="clear-btn" tabindex="-1" title="Clear filter">✕</button>';
  ...
  clearItemText(item) {
    const keep = [
      ...item.querySelectorAll(
        ':scope > .method-gutter, :scope > .override-arrow, :scope > .session-indicator',
      ),
    ];
    item.textContent = '';
