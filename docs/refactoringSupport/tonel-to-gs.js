...
function main() {
  ...
  let dict = 'UserGlobals', headerFile = null, out = null;
  const dirs = [];
  ...
  if (!out || dirs.length === 0) {
    console.error('usage: tonel-to-gs.js --dict D --header H --out O <packageDir>...');
    process.exit(2);
  }
  ...
  for (const e of extensions) for (const m of e.methods) parts.push(emitMethod(e.name, m));
