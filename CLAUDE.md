...
<!-- Maintainer note (stripped from agent context): this map is intentionally lean. Deeper per-subsystem detail lives in .claude/rules/ (GCI, MCP, tests, client tests); all auto-load by path when relevant files are opened. Don't re-expand the map above. -->
