...
4. **Set up the GemStone test instance** (integration tests are part of `npm test` and require this):

   ```sh
   npm run test:setup
   ```

   This provisions a local GemStone instance and writes `.env.test` with the connection details. To pin a specific version instead of the latest, pass it as an argument: `npm run test:setup -- 3.7.2`.

   **Supported on Linux and macOS (Apple Silicon) only — Windows is not yet supported.**

   > **Destructive:** re-running this command stops any running stone, resets the database to a pristine state, and overwrites `.env.test`.
...
If you want to run the integration tests against your own GemStone instance instead of the one provisioned by `test:setup`, you can override the connection details via environment files. Vite loads these automatically when running tests; later files take precedence over earlier ones:
